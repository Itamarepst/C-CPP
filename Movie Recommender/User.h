...
class User
{
 private:
  ...
  // A hash table of movie and its rankings
  rank_map _rankings;

  // a pointer to the recommendation system the user uses
  std::shared_ptr<RecommendationSystem> _rs_ptr;

 public:
  /**
   * Constructor for the class
   */
  User (std::string name, rank_map rankings, const
   ...
   */
  void add_movie_to_rs (const std::string &name, int year, const
   ...
   */
  friend std::ostream &operator<< (std::ostream &os, const User &man);
